...
img_pool = cv2.resize(img_pool,(int(newx),int(newy)))
...
for cnt_pool in contours_pool:
    if len(cnt_pool) > largest_area:
        largest_contour_index = index
        largest_area = len(cnt_pool) 
...
while(cap.isOpened() and frame_num < 3323):
        ...
        if(person_count == 0):
            ...
            for num in range(1,person_count+1):
                if( person_flag[num] != 0):
                    ...
                    x_end = int(person_roi[num][0]+person_roi[num][2])
                    y_end = int(person_roi[num][1]+person_roi[num][3])
                    # check whether each person is in pool
                    cv2.circle(img,(x_end,y_end),2,(0,0,255),-1)
                    dist = cv2.pointPolygonTest(contours_pool[largest_contour_index],(x_end,y_end),False)
                    if (dist == 1 or dist == 0): 
                    # if(dist == 1):
                        person_pool_count += 1
                    #print (dist)
    ...
    cv2.drawContours(img,[contours_pool[largest_contour_index]],-1,(255,0,0),-1)
    # text for counting
    ...
    cv2.putText(img,"In Pool:%d" %(person_pool_count), (10, img.shape[0] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.35, (0, 0, 255), 1)
